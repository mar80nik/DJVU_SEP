...
{
	Buffer=Original=0; FullName=name;
	BckgRgn=MyRect(0,0,2,2); IsBckgFile=false;
	ZonesActive.Eql(0);
	Zones.RemoveAll();	
	ProcessFullName();
	SetBckgFile(Path+Name+".tif");
	GetPicDim(name,Ext);

	Parent=parent;
	Buffer=new BMPanvas(Parent->GetDC());	
	Original=new BMPanvas(Parent->GetDC());	

	CropZone.enable=false; CropZone.Rect=CRect(CPoint(0,0),PicDim);
	CropZone.cur_color=RGB(255,255,0);
	CropZone.brsh.CreateSolidBrush(CropZone.cur_color);
	CropZone.Pen1.CreatePen(PS_SOLID,1,CropZone.cur_color);	
	Param.dd=CPoint(2,2); 
}
...
int DjvuPic::SetBckgFile(CString &file)
{
	CString SearchPattern=file; HANDLE SearchHandle;
	WIN32_FIND_DATA FindFileData;
	if( (SearchHandle=FindFirstFile(LPCSTR(SearchPattern),&FindFileData) )==INVALID_HANDLE_VALUE)
	{			
		IsBckgFile=false; BckgFile="";
	}
	else
	{
		IsBckgFile=true; BckgFile=file;	
	}	
	return IsBckgFile;	
}
...
{	
	int err=ERR_OK;
/*
	if(Original->ColorType==BMPanvas::BW)
	{			
		BMPINFO temp=Original->INFO; temp.bmiHeader.biBitCount=8; 
		Buffer->Destroy();
		Buffer->Create(&temp); 
	...
	{
		Buffer->Create(Original,Original->Rgn);		
	}
	*/
		BMPINFO temp=Original->INFO; temp.bmiHeader.biBitCount=8; 
		Buffer->Destroy();
	...
	if(IsBckgFile && check)
	{
		CImage TT; TT.Load(BckgFile);
		//TT.BitBlt(Buffer->GetSafeHdc(),0,0);
		TT.BitBlt(Buffer->GetDC(),0,0);
		//Buffer->ColorType=BMPanvas::BW;
	}
	else
	{
		Original->CopyTo(Buffer,CPoint(0,0),Original->Rgn); 		
	}	
...
void DjvuPic::GetPicDim(const CString& name, const CString& Ext)
{
	bool error=true; BMPINFO temp;
	if(Ext==".jpg")
	{
		temp=MyJPEG::ReadJPGHeader(name); error=false;		
	}
	if(Ext==".bmp")
	{
        CFile ttt;
		if(ttt.Open(name,CFile::modeRead))
		{
			...
			ttt.Read(&temp,sizeof(BMPINFO)); error=false;			
			ttt.Close();
		}
	}
	if(!error) PicDim.SetSize(temp.bmiHeader.biWidth,temp.bmiHeader.biHeight);	
}
