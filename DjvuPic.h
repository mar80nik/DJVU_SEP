...
protected:
	...
	int ProcessFullName();
	int SetBckgFile(CString&);

public:
	MyRect BckgRgn; MyRectangle CropZone; 	
	BMPanvas *Buffer, *Original;
	GausianBlurParam GBparam;
	CString FullName, Path,Name, Ext, BckgFile;	bool IsBckgFile;
	RotateFilterParam CenterRotate;
	CSize PicDim;

	DjvuPic(CString,OutputWnd*);	
	virtual ~DjvuPic();
	...
	void GetPicDim(const CString& name, const CString& Ext);
	int CheckDjvu(void);
};
