...
//	CoordCanvas.SetWindowOrg(ActiveWnd.TopLeft());
	...
	case ZONE_MOVE:
		...
		break;
	case CROP_MOVE:
		tt0=tt=CurPic->CropZone.Rect; diff=CurPoint-MM0; 
		tt.OffsetRect(diff.x,0); 
		if(tt.left<0 || tt.right>CurPic->PicDim.cx) {tt=tt0;}
		tt0=tt; tt.OffsetRect(0,diff.y); 
		if(tt.top<0 || tt.bottom>CurPic->PicDim.cy) {tt=tt0;}
		if(CurPic->CropZone.Rect!=tt)
		{
			CurPic->CropZone.Rect=tt;            		
			DrawMode.Set(FRG);Draw();
		}
		break;
	case PIC_MOVE:
